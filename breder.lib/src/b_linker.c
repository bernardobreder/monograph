...
b_linker_t* b_linker_new () {
	b_linker_t* self = (b_linker_t*)b_memory_alloc0 (sizeof(b_linker_t));
	return self;
}
...
static b_linker_node_t* b_linker_node_new (b_vm_t* vm, b_linker_node_t *back, b_linker_node_t *next, void* data) {
	b_linker_node_t* node = (b_linker_node_t*)b_memory_alloc (vm, sizeof(b_linker_node_t));
	node->back = back;
	node->next = next;
	node->data = data;
	return node;
...
void* b_linker_get_last (b_linker_t* self) {
	if (self->last) return self->last->data;
	else return 0;
}
