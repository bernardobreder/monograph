...
public class Break extends Command implements ICommand {
	...
	public void check(Context context) throws ParseException {
		{
			...
			while (!(node instanceof While || node instanceof Repeat || node instanceof For)) {
				node = node.getParent();
				if (node == null) {
					throw new BrederJRuntimeException(context, getToken(), "break without generic loop statement");
				}
			}
		}
	}
	...
	public void build(Context context, BinaryOutputStream output) throws IOException {
		int index = context.getMethod().peekEndRepeat() + context.getMethod().peekBeginLoop();
